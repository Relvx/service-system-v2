  ...
  <Layout>
    <div>
      ...
      <div v-else class="space-y-4">
        <button v-for="d in defects" :key="d.id" type="button" class="card hover:shadow-md transition-shadow cursor-pointer w-full text-left" @click="selectedDefect = d">
      ...
      <BaseModal v-if="selectedDefect" :title="selectedDefect.title" @close="selectedDefect = null">
        <div class="p-6 space-y-3 text-sm">
          ...
          <div class="pt-3 border-t">
            <label for="defect-status" class="block text-sm font-medium text-gray-700 mb-1">Изменить статус</label>
            <div class="flex gap-2">
              <select id="defect-status" v-model="newStatus" class="input flex-1">
                <option v-for="s in cfg.defectStatuses" :key="s.code" :value="s.code">{{ s.display_name }}</option>
              </select>
...
import { AlertTriangle, Building2, Users, Calendar } from 'lucide-vue-next'
