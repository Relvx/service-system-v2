  ...
  <Layout>
    <div class="flex h-[calc(100vh-8rem)] gap-4">
      ...
      <div class="w-80 flex flex-col gap-4 overflow-hidden">
        <div class="card">
          <div class="relative">
            <Search class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" aria-hidden="true" />
            <input v-model="search" type="text" placeholder="Поиск объекта..." aria-label="Поиск объекта" class="input pl-10" @input="filterSites" />
          </div>
        </div>
          ...
          <div v-else class="space-y-2">
            ...
            >
              <div class="flex items-center gap-2">
                <Building2 class="w-4 h-4 text-gray-500 flex-shrink-0" aria-hidden="true" />
                <div class="min-w-0">
                  <p class="font-medium text-sm text-gray-900 truncate">{{ s.title }}</p>
                  <p class="text-xs text-gray-500 truncate">{{ s.address }}</p>
                  <p v-if="s.client_name" class="text-xs text-gray-600 truncate">{{ s.client_name }}</p>
                </div>
              </div>
            </button>
