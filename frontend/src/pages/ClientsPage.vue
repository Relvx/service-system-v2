<template>
  <Layout>
    <div>
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-3xl font-bold text-gray-900">Клиенты</h1>
          <p class="text-gray-600 mt-1">Всего клиентов: {{ clients.length }}</p>
        </div>
        <button @click="openCreate" class="btn btn-primary flex items-center">
          <Plus class="w-5 h-5 mr-2" />Добавить клиента
        </button>
      </div>

      <div class="card mb-6">
        <div class="flex gap-4">
          <div class="flex-1 relative">
            <Search class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
            <input
              v-model="search"
              @keyup.enter="loadClients"
              type="text"
              placeholder="Поиск по названию, ИНН, контактному лицу..."
              class="input pl-10"
            />
          </div>
          <button @click="loadClients" class="btn btn-primary">Найти</button>
        </div>
      </div>

      <div v-if="loading" class="flex items-center justify-center h-64">
        <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-600"></div>
      </div>

      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="c in clients" :key="c.id" class="card hover:shadow-md transition-shadow flex flex-col">
          <div class="flex items-start justify-between mb-3">
            <div class="flex items-center">
              <div class="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center mr-3">
                <Building2 class="w-5 h-5 text-primary-600" />
              </div>
              <div>
                <h3 class="font-semibold text-gray-900">{{ c.name }}</h3>
                <p v-if="c.inn" class="text-sm text-gray-500">ИНН: {{ c.inn }}</p>
              </div>
            </div>
          </div>

          <div v-if="c.contact_person" class="flex items-center text-sm text-gray-600 mb-2">
            <Phone class="w-4 h-4 mr-2" />{{ c.contact_person }}
          </div>
          <div v-if="c.contacts" class="flex items-center text-sm text-gray-600 mb-3">
            <Mail class="w-4 h-4 mr-2" />{{ c.contacts.split(',')[0] }}
          </div>
          <p v-if="c.notes" class="text-sm text-gray-500 mb-3 border-t pt-3">{{ c.notes }}</p>

          <div class="flex gap-2 mt-auto pt-3 border-t">
            <button @click="openEdit(c)" class="flex-1 btn btn-secondary text-sm py-2 flex items-center justify-center">
              <Edit class="w-4 h-4 mr-1" />Изменить
            </button>
            <button @click="deleteConfirm = c" aria-label="Удалить клиента" class="btn bg-red-50 text-red-600 hover:bg-red-100 text-sm py-2 px-3">
              <Trash2 class="w-4 h-4" aria-hidden="true" />
            </button>
          </div>
        </div>
      </div>

      <div v-if="!loading && clients.length === 0" class="text-center py-12">
        <Building2 class="w-16 h-16 text-gray-300 mx-auto mb-4" />
        <h3 class="text-lg font-medium text-gray-900 mb-2">Клиенты не найдены</h3>
      </div>

      <!-- Create / Edit Modal -->
      <BaseModal v-if="modalOpen" :title="editing ? 'Редактировать клиента' : 'Добавить клиента'" @close="modalOpen = false">
        <form @submit.prevent="handleSave" class="p-6 space-y-4">
          <div>
            <label for="client-name" class="block text-sm font-medium text-gray-700 mb-1">Название *</label>
            <input id="client-name" v-model="form.name" required class="input" placeholder='ООО "Название"' />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label for="client-inn" class="block text-sm font-medium text-gray-700 mb-1">ИНН</label>
              <input id="client-inn" v-model="form.inn" class="input" placeholder="1234567890" />
            </div>
            <div>
              <label for="client-kpp" class="block text-sm font-medium text-gray-700 mb-1">КПП</label>
              <input id="client-kpp" v-model="form.kpp" class="input" placeholder="123456789" />
            </div>
          </div>
          <div>
            <label for="client-contact" class="block text-sm font-medium text-gray-700 mb-1">Контактное лицо</label>
            <input id="client-contact" v-model="form.contact_person" class="input" placeholder="Иванов Иван" />
          </div>
          <div>
            <label for="client-contacts" class="block text-sm font-medium text-gray-700 mb-1">Контакты</label>
            <input id="client-contacts" v-model="form.contacts" class="input" placeholder="8-495-123-45-67" />
          </div>
          <div>
            <label for="client-notes" class="block text-sm font-medium text-gray-700 mb-1">Заметки</label>
            <textarea id="client-notes" v-model="form.notes" class="input" rows="3" placeholder="Дополнительная информация..." />
          </div>
          <div class="flex justify-end gap-3 pt-4">
            <button type="button" @click="modalOpen = false" class="btn btn-secondary">Отмена</button>
            <button type="submit" :disabled="saving" class="btn btn-primary disabled:opacity-50">
              {{ saving ? 'Сохранение...' : (editing ? 'Сохранить' : 'Создать') }}
            </button>
          </div>
        </form>
      </BaseModal>

      <!-- Delete Confirm -->
      <BaseModal v-if="deleteConfirm" title="Удалить клиента?" max-width="max-w-md" @close="deleteConfirm = null">
        <div class="p-6">
          <p class="text-gray-600 mb-6">Вы уверены? Это действие нельзя отменить.</p>
          <div class="flex justify-end gap-3">
            <button @click="deleteConfirm = null" class="btn btn-secondary">Отмена</button>
            <button @click="handleDelete" class="btn bg-red-600 text-white hover:bg-red-700">Удалить</button>
          </div>
        </div>
      </BaseModal>
    </div>
  </Layout>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { Search, Plus, Building2, Phone, Mail, Edit, Trash2 } from 'lucide-vue-next'
import Layout from '../components/Layout.vue'
import BaseModal from '../components/BaseModal.vue'
import { clientsAPI } from '../services/api.js'

const clients = ref([])
const loading = ref(true)
const search = ref('')
const modalOpen = ref(false)
const editing = ref(null)
const deleteConfirm = ref(null)
const saving = ref(false)
const form = ref({ name: '', inn: '', kpp: '', contact_person: '', contacts: '', notes: '' })

async function loadClients() {
  loading.value = true
  try {
    const res = await clientsAPI.getAll({ search: search.value, active_only: true })
    clients.value = res.data
  } finally {
    loading.value = false
  }
}

function openCreate() {
  editing.value = null
  form.value = { name: '', inn: '', kpp: '', contact_person: '', contacts: '', notes: '' }
  modalOpen.value = true
}

function openEdit(c) {
  editing.value = c
  form.value = { name: c.name, inn: c.inn || '', kpp: c.kpp || '', contact_person: c.contact_person || '', contacts: c.contacts || '', notes: c.notes || '' }
  modalOpen.value = true
}

async function handleSave() {
  saving.value = true
  try {
    if (editing.value) {
      await clientsAPI.update(editing.value.id, form.value)
    } else {
      await clientsAPI.create(form.value)
    }
    modalOpen.value = false
    await loadClients()
  } catch (e) {
    alert('Ошибка: ' + (e.response?.data?.detail || e.message))
  } finally {
    saving.value = false
  }
}

async function handleDelete() {
  try {
    await clientsAPI.delete(deleteConfirm.value.id)
    deleteConfirm.value = null
    await loadClients()
  } catch (e) {
    alert('Ошибка: ' + (e.response?.data?.detail || e.message))
  }
}

onMounted(loadClients)
</script>
