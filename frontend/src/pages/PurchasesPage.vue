<template>
  <Layout>
    <div>
      ...
      <!-- Create Modal -->
      <BaseModal v-if="modalOpen" title="Добавить закупку" @close="modalOpen = false">
        <form @submit.prevent="handleSave" class="p-6 space-y-4">
          <div>
            <label for="purchase-item" class="block text-sm font-medium text-gray-700 mb-1">Наименование *</label>
            <input id="purchase-item" v-model="form.item" required class="input" placeholder="Насос циркуляционный Grundfos" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label for="purchase-qty" class="block text-sm font-medium text-gray-700 mb-1">Кол-во</label>
              <input id="purchase-qty" v-model="form.qty" type="number" step="0.01" min="0" class="input" />
            </div>
            <div>
              <label for="purchase-due" class="block text-sm font-medium text-gray-700 mb-1">Срок</label>
              <input id="purchase-due" v-model="form.due_date" type="date" class="input" />
            </div>
          </div>
          <div>
            <label for="purchase-notes" class="block text-sm font-medium text-gray-700 mb-1">Заметки</label>
            <textarea id="purchase-notes" v-model="form.notes" class="input" rows="2" />
          </div>
...
import { Plus, ShoppingCart, Building2, AlertTriangle, Calendar } from 'lucide-vue-next'
