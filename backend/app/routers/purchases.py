...
async def get_purchases(
    ...
    if not show_archived:
        stmt = stmt.where(Purchase.is_archived == False)
    if status:
...
@router.patch("/{purchase_id}/archive", response_model=PurchaseOut)
...
@router.patch("/{purchase_id}/unarchive", response_model=PurchaseOut)
async def unarchive_purchase(
    purchase_id: int,
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
